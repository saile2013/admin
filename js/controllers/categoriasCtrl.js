angular.module("sop").controller("categoriasCtrl", function($scope, $cookies, $state, servicosAPI){
	...
	$scope.seleEstato = function(){
		$state.go('search-categorias', {text:null, status:$scope.estato.value});
	};

	$scope.buscar = function(){
		if($scope.buscando){
			$state.go('search-categorias', {text:$scope.buscando, status:null});
		}
	};
