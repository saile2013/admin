angular.module("sop").controller("cotacaoLogisticaCtrl", function($scope, $cookies, $state, servicosAPI){
	...
	$scope.verre = function() {
		$state.go('search-cotacao-logistica', {tienda:$scope.tienda, transportadora:null, text:null, begin:null, end:null, hra1:null, hra2:null});
	};

	$scope.trans = function(){
		$state.go('search-cotacao-logistica', {tienda:null, transportadora:$scope.transportadora, text:null, begin:null, end:null, hra1:null, hra2:null});
	};
	...
	$scope.verDatas = function() {
		if($scope.data2){
			...
			$state.go('search-cotacao-logistica', {tienda:null, text:null, begin:data1, end:data2, hra1:hra1, hra2:hra2});
		}
	};

	$scope.viewGet = function(){

		servicosAPI.getLogistica($scope.pag).then(function(result) {
			console.log(result);
	...
	$scope.contador2 = function(id) {
		...
		document.getElementById('dtl'+id).innerHTML = 'Máximo 25 caracteres requeridos. caracteres: ('+valor.length+')';
	}
	...
	$scope.buscar = function(){
		if($scope.buscando){
			$state.go('search-cotacao-logistica', {tienda:null, transportadora:null, text:$scope.buscando, begin:null, end:null, hra1:null, hra2:null});
		}
	};
