angular.module("sop").controller("editarCategoriasCtrl", function($scope, $stateParams, $cookies, $state, servicosAPI){
	...
	$scope.viewCategorias = function(){
		servicosAPI.dtlCategorias($scope.idCat).then(function (result) {
			...
			$scope.img_category = result.data.data.img_category;
			$scope.banner_mobile = result.data.data.banner;
			$scope.sort = result.data.data.sort;
			$scope.primary_color = result.data.data.primary_color;
			$scope.secondary_color = result.data.data.secondary_color;
	...
	$scope.carrega2 = function(){
		...
		setTimeout(function(){
			console.log($scope.imagem2.naturalWidth, $scope.imagem2.naturalHeight);
			if((parseInt($scope.imagem2.naturalWidth) != 1920) || (parseInt($scope.imagem2.naturalHeight) != 400)){
	...
	$scope.salvar = function(){
		...
		if(!$scope.name){
			...
			servicosAPI.putCategorias($scope.idCat, dados).then(function (result) {
				$scope.botao = true;
