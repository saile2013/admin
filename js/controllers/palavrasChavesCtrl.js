angular.module("sop").controller("palavrasChavesCtrl", function($scope, $cookies, $state, servicosAPI){
	...
	servicosAPI.getWords($scope.pag).then(function(result){
	...
	$scope.buscar = function(){
		if($scope.buscando){
			$state.go('search-palavras-chaves', {text:$scope.buscando, begin:null, end:null});
		}
	}
	...
	$scope.seleData = function(){
		...
		$state.go('search-palavras-chaves', {text:null, begin:data1, end:data2});
