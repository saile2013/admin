angular.module("sop").controller("perfilPalavrasCtrl", function($scope, $cookies, $state, $stateParams, servicosAPI){
	...
	$scope.ordernnn = function(value, aparence){
		...
		if(value == 1 || value==2){
			servicosAPI.getOrdDtlSearch($scope.id, 'id', value, $scope.buscando, 'NaN-NaN-NaN', 'NaN-NaN-NaN', $scope.pag).then(function(result){
				tiendas.splice(0);
				$scope.pag = 1;
			...
			servicosAPI.getOrdDtlOrigin($scope.id, 'origin', value, $scope.buscando, 'NaN-NaN-NaN', 'NaN-NaN-NaN', $scope.pag).then(function(result){
				tiendas.splice(0);
				$scope.pag = 1;
			...
			servicosAPI.getOrdDtlType($scope.id, 'type', value, $scope.buscando, 'NaN-NaN-NaN', 'NaN-NaN-NaN', $scope.pag).then(function(result){
				tiendas.splice(0);
				$scope.pag = 1;
			...
			servicosAPI.getOrdDtlResult($scope.id, 'result', value, $scope.buscando, 'NaN-NaN-NaN', 'NaN-NaN-NaN', $scope.pag).then(function(result){
				tiendas.splice(0);
				$scope.pag = 1;
			...
			servicosAPI.getOrdDtlCreado($scope.id, 'created_at', value, $scope.buscando, 'NaN-NaN-NaN', 'NaN-NaN-NaN', $scope.pag).then(function(result){
				tiendas.splice(0);
				$scope.pag = 1;
	...
	$scope.buscar = function(){
		if($scope.buscando){
			$state.go('search-perfil-palavras', {id:$scope.id, text:$scope.buscando, begin:null, end:null});
		}
	}
	...
	$scope.seleData = function(){
		...
		$state.go('search-perfil-palavras', {id:$scope.id, text:null, begin:data1, end:data2});
	}
