angular.module("sop").controller("porcidadeEstatisticasCtrl", function($scope, $state, $cookies, $state, $stateParams, servicosAPI){
	...
	servicosAPI.getPorCidades($scope.id).then(function(resulT1){
		console.log(resulT1.data.data);
		$scope.ages = resulT1.data.data;
		$scope.mensagens = '';
		$scope.loader = false;
	}).catch(function(err, status){
		if(err.status == '400'){
