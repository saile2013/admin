angular.module("sop").controller("restaurantesOrdenesCtrl", function($scope, $state, $cookies, $state, servicosAPI){
	...
	servicosAPI.getRestaurantesOrdenes($scope.pag).then(function(result){
	...
	$scope.seleStatus = function(){
		$state.go('search-restaurantes-ordenes', {text:null, status:$scope.estatu.value, begin:null, end:null});
	};
	...
	$scope.seleData = function(){
		...
		$state.go('search-restaurantes-ordenes', {text:null, status:null, begin:data1, end:data2});
	}
	...
	$scope.buscar = function(){
		if($scope.buscando){
			$state.go('search-restaurantes-ordenes', {text:$scope.buscando, status:null, begin:null, end:null});
		}
	}
