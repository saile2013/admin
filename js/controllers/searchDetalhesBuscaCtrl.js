angular.module("sop").controller("searchDetalhesBuscaCtrl", function($scope, $cookies, $state, $stateParams, servicosAPI){
	...
	servicosAPI.searchDtlWords($scope.pag, $scope.text).then(function(result){
	...
	$scope.buscar = function(){
		if($scope.buscando){
			$state.go('search-detalhes-busca', {text:$scope.buscando, begin:$scope.begin, end:$scope.end});
		}
	}
	...
	$scope.seleData = function(){
		...
		$state.go('search-detalhes-busca', {text:$scope.buscando, begin:data1, end:data2});
	}
