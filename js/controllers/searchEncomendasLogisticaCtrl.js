angular.module("sop").controller("searchEncomendasLogisticaCtrl", function($scope, $cookies, $state, $stateParams, servicosAPI){
	...
	$scope.verre = function() {
		$state.go('search-encomendas-logistica', {tienda:$scope.tienda, transportadora:$scope.transportadora, text:$scope.text, begin:$scope.begin, end:$scope.end, hra1:$scope.hra1, hra2:$scope.hra2});
	};
	...
	$scope.verDatas = function() {
		if($scope.data2){
			...
			$state.go('search-encomendas-logistica', {tienda:$scope.tienda, transportadora:$scope.transportadora, text:$scope.text, begin:data1, end:data2, hra1:hra1, hra2:hra2});
		}
	};

	$scope.viewGet = function() {
		servicosAPI.search2EncomendasLogistica($scope.tienda, $scope.transportadora, $scope.begin, $scope.end, $scope.hra1, $scope.hra2, $scope.text, $scope.pag).then(function(result) {			
	...
	$scope.buscar = function(){
		if($scope.buscando){
			$state.go('search-encomendas-logistica', {tienda:$scope.tienda, transportadora:$scope.transportadora, text:$scope.buscando, begin:$scope.begin, end:$scope.end, hra1:$scope.hra1, hra2:$scope.hra2});
		}
	};
