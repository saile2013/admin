angular.module("sop").controller("searchPedidosCtrl", function($scope, $cookies, $state, $stateParams, servicosAPI){
	...
	if($stateParams.status == 'canceled'){
		...
		var st = 4;
	}else if($stateParams.status == 'delivered'){
		var st = 5;
	}else{
		var st = 0;
	}
	...
	servicosAPI.searchPedidos($scope.text, $scope.status, $scope.begin, $scope.end, $scope.pag).then(function(result){
		...
		$scope.maisPag = function(){
			$scope.apa = false;
			$scope.pag += 1;
			servicosAPI.searchPedidos($scope.text, $scope.status, $scope.begin, $scope.end, $scope.pag).then(
			function(result){
				for(var i=0; i<result.data.data.length; i++){
					pedidos.push(result.data.data[i]);
					$scope.apa = true;
	...
	$scope.seleData = function(){
		...
		if($scope.ate){
			...
			$state.go('search-pedidos', {text:$scope.text, status:$scope.status, begin:data1, end:data2});
		}
	}

	$scope.seleStatus = function(){
		$state.go('search-pedidos', {text:$scope.text, status:$scope.estatu.value, begin:$scope.begin, end:$scope.end});
	};
	...
	$scope.buscar = function(){
		if($scope.buscando){
			$state.go('search-pedidos', {text:$scope.buscando, status:$scope.status, begin:$scope.begin, end:$scope.end});
		}
	}
