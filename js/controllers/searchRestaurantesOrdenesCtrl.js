angular.module("sop").controller("searchRestaurantesOrdenesCtrl", function($scope, $state, $cookies, $state, $stateParams, servicosAPI){
	...
	servicosAPI.searchRestaurantesOrdenes($scope.text, $scope.status, $scope.begin, $scope.end, $scope.pag).then(function(result){
	...
	$scope.seleStatus = function(){
		...
		$state.go('search-restaurantes-ordenes', {text:$scope.buscando, status:$scope.estatu.value, begin:data1, end:data2});
	};
	...
	$scope.seleData = function(){
		...
		$state.go('search-restaurantes-ordenes', {text:$scope.buscando, status:$scope.status, begin:data1, end:data2});
	}
