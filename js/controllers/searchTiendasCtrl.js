angular.module("sop").controller("searchTiendasCtrl", function($scope, $cookies, $stateParams, $state, servicosAPI){
	...
	$scope.seleEstato = function(){
		$state.go('search-tiendas', {text:$scope.text, status:$scope.estato.value});
	};

	$scope.buscar = function(){
		if($scope.buscando){
			$state.go('search-tiendas', {text:$scope.buscando, status:$scope.status});
		}
	}
